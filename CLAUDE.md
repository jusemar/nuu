# CLAUDE.md — Regras do Projeto (Loja Virtual)

> Documento vivo, derivado de `regras.md`. O Claude Code deve seguir estas regras **sem exceção**.
> `regras.md` continua sendo a fonte original; este arquivo é a versão adaptada ao Claude Code.

---

## Visão geral

- Projeto: loja virtual em **Next.js (App Router)** + TypeScript + Drizzle ORM + Tailwind CSS + shadcn/ui.
- Arquitetura: **Feature-based (modular por domínio)**.
- Idioma de código e nomenclatura: **português** (ver "Convenções de nomenclatura").

---

## 🤖 Comportamento esperado do Claude Code

1. **Planeje antes de mudanças grandes.** Antes de alterar algo grande, explique rapidamente o plano (use o modo de plano quando a mudança for ampla). Depois execute sem pedir aprovação a cada comando.

2. **Após aprovação, ao reportar:**
   - passo a passo do que foi feito;
   - caminho completo de cada arquivo alterado ou criado (ex: `src/features/produtos/queries/buscar-produto.ts`), respeitando a arquitetura real do domínio e o padrão de organização já existente no projeto;
   - código sempre comentado e explicativo, de forma que qualquer iniciante consiga entender ao ler.

3. **Atue como engenheiro de software sênior especialista em React/Next.js:**
   - código limpo;
   - separação de responsabilidades;
   - escalabilidade.

4. **Interfaces sempre responsivas** para qualquer tamanho de tela.

5. **Reutilizar antes de criar.** Priorizar sempre componentes, padrões e soluções já validadas e amplamente utilizadas pela comunidade antes de criar componentes customizados do zero.
...
6. **UX primeiro em funcionalidades administrativas.** Para novas funcionalidades administrativas, iniciar sempre pela experiência do usuário.
...
7. **Autonomia do agente**
   ...
   - **Ao final, sempre validar com:**
     - `npx tsc --noEmit` (não existe script `typecheck` no `package.json`);
     - `npm run lint`;
...
**Regra prática:**
👉 Até 3 arquivos → pode ficar direto na pasta
...
- **Tabelas:** nome em **snake_case plural** — ex: `products`, `order_items`.
- **Campos obrigatórios:** `id`, `createdAt`, `updatedAt`.
- **Chaves estrangeiras:** sempre explícitas (`categoryId`, `userId`).
...
- **Não usar nomes em inglês para:** arquivos, variáveis, funções, schemas, componentes, tipos, actions, queries, tabelas, enums.
...
Evitar: `shipping`, `freight`, `dimensions`, `weight`, `delivery`, `externalShipping`.

Exceção: APIs externas, bibliotecas, nomes obrigatórios do framework, nomes oficiais de terceiros.
...
- API Routes → apenas webhooks e integrações externas.
...
1. Toda página pública deve usar `generateMetadata`.
...
3. Conteúdo renderizado no servidor (SSR ou SSG).
   ❌ evitar conteúdo crítico só no client
4. HTML semântico: 1 `<h1>` por página, hierarquia correta (`h2`, `h3`).
5. Performance: imagens otimizadas, evitar JS desnecessário, lazy loading quando possível.
6. Dados estruturados: JSON-LD (produtos, preços, avaliações).
...
2. Ordem das classes: layout → spacing → typography → colors → states.
...
Usar CSS para: animações complexas, scrollbar, casos específicos.
...
- validar sessão em todas as actions;
...
- paginação desde o início;
- evitar hardcode;
- componentes < 200 linhas;
- seguir a ordem: **banco → types → actions → UI**.
...
npx drizzle-kit generate  # gerar migration após alterar schema
```
...
👉 Código deve ser: **escalável, organizado, reutilizável e fácil de manter.**
