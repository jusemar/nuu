import { pgTable, foreignKey, unique, uuid, text, timestamp, integer, boolean, jsonb, index, serial, varchar, numeric, time, date, pgEnum } from "drizzle-orm/pg-core"
...
export const category = pgTable("category", {
	...
	index("category_parent_idx").using("btree", table.parentId.asc().nullsLast().op("uuid_ops")),
...
export const states = pgTable("states", {
	...
	unique("states_uf_unique").on(table.uf),
]);
...
export const productDeliveryMethods = pgTable("product_delivery_methods", {
	id: integer().primaryKey().generatedAlwaysAsIdentity({ name: "product_delivery_methods_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	productId: uuid("product_id").notNull(),
...
export const productSuppliers = pgTable("product_suppliers", {
	id: integer().primaryKey().generatedAlwaysAsIdentity({ name: "product_suppliers_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	productId: uuid("product_id").notNull(),
...
export const productVariantImage = pgTable("product_variant_image", {
	id: uuid().defaultRandom().primaryKey().notNull(),
...
export const cepsEspecificos = pgTable("ceps_especificos", {
	...
	unique("ceps_especificos_cep_unique").on(table.cep),
]);
...
export const regiaoBairros = pgTable("regiao_bairros", {
	id: serial().primaryKey().notNull(),
...
export const configHorario = pgTable("config_horario", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	...
	index("config_horario_dias_idx").using("btree", table.diasFuncionamento.asc().nullsLast().op("text_ops")),
]);
...
export const feriados = pgTable("feriados", {
	...
	index("feriados_data_idx").using("btree", table.data.asc().nullsLast().op("date_ops")),
]);

export const prazosRetirada = pgTable("prazos_retirada", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	nome: text().notNull(),
	slug: text().notNull(),
	...
	index("prazos_retirada_slug_idx").using("btree", table.slug.asc().nullsLast().op("text_ops")),
	unique("prazos_retirada_slug_unique").on(table.slug),
]);

export const produtoRetirada = pgTable("produto_retirada", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	productId: uuid("product_id").notNull(),
	...
	createdId: timestamp("created_id", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
}, (table) => [
	index("produto_retirada_modalidade_idx").using("btree", table.modalidadeId.asc().nullsLast().op("uuid_ops")),
	index("produto_retirada_ponto_idx").using("btree", table.pontoColetaId.asc().nullsLast().op("uuid_ops")),
	index("produto_retirada_product_idx").using("btree", table.productId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
			columns: [table.productId],
			foreignColumns: [product.id],
...
export const pontosColeta = pgTable("pontos_coleta", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	nome: text().notNull(),
	slug: text().notNull(),
	...
	index("pontos_coleta_cidade_idx").using("btree", table.cidade.asc().nullsLast().op("text_ops")),
	index("pontos_coleta_slug_idx").using("btree", table.slug.asc().nullsLast().op("text_ops")),
