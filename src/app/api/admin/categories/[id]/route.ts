...
export async function DELETE(
  ...
  try {
    ...
    if (hasChildren && hasChildren.length > 0) {
      console.log(
        `[API DELETE] Categoria ${id} tem subcategorias, cannot delete`,
      );
      return NextResponse.json(
        {
          success: false,
    ...
    console.log(
      `[API DELETE] Sem subcategorias, prosseguindo com delete ${type}`,
    );
    ...
    if (type === "soft") {
      ...
      console.log(
        `[API DELETE] Executando HARD DELETE (deletar permanentemente)`,
      );
      const result = await db
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            'Tipo de operação inválido. Use "soft", "hard" ou "restore".',
        },
        { status: 400 },
      );
    }
