...
export const productTable = pgTable("product", {
  ...
  // Códigos
  sku: text("sku").notNull().default(sql`gen_random_uuid()`),
   ...
   */
  preferredSupplierIds: jsonb("preferred_supplier_ids").$type<string[]>().default([]),
   ...
   */
  allowedDeliveryMethodIds: jsonb("allowed_delivery_method_ids").$type<number[]>().default([]),
