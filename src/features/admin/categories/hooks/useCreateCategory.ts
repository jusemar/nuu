  ...
  >({
    mutationFn: async (categoryData) => {
      ...
      const dataToSend = {
        ...
        isActive:
          (categoryData as any).isActive !== undefined
            ? (categoryData as any).isActive
            : true,
      };

      return await createCategory(dataToSend);
    },
    ...
    onSuccess: (createdCategory, variables, context) => {
      ...
      queryClient.setQueryData<Category[]>(categoryKeys.lists(), (old) =>
        old?.map((cat) => (cat.id.startsWith("temp-") ? createdCategory : cat)),
      );
      ...
      toast.success(toastConfig.success.title, {
        ...
        style: {
          backgroundColor: "#22c55e", // Verde igual ao seu botão
    ...
    onError: (error, variables, context) => {
      ...
      if ((context as any)?.previousCategories) {
        queryClient.setQueryData(
          categoryKeys.lists(),
          (context as any).previousCategories,
        );
      }
  ...
  >({
    ...
    onMutate: async (categories) => {
      ...
      const optimisticCategories = categories.map((cat, index) => ({
        ...
        isActive:
          (cat as any).isActive !== undefined ? (cat as any).isActive : true,
      ...
      queryClient.setQueryData<Category[]>(categoryKeys.lists(), (old) =>
        old ? [...optimisticCategories, ...old] : optimisticCategories,
      );
