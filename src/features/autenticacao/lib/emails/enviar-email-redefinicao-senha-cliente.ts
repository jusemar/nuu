...
export async function enviarEmailRedefinicaoSenhaCliente({
  ...
  const resultado = await obterResend().emails.send({
    ...
    to: destinatario,
    subject: `Redefina sua senha da ${DADOS_EMPRESA.marca}`,
    text: `Recebemos uma solicitação para redefinir sua senha da ${DADOS_EMPRESA.marca}. Use este link em até 30 minutos: ${urlRedefinicao}\n\nSe você não solicitou a alteração, ignore este email.`,
    html: `
      <div style="font-family:Arial,sans-serif;color:#0f172a;line-height:1.6">
        <h1 style="font-size:20px">Crie uma nova senha</h1>
        <p>Recebemos uma solicitação para redefinir sua senha da ${DADOS_EMPRESA.marca}.</p>
