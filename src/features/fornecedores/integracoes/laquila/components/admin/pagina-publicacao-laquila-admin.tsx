...
export function PaginaPublicacaoLaquilaAdmin({
  ...
  function publicarConfirmados() {
    ...
    iniciarTransicao(async () => {
      const resultado = await publicarProdutosLaquila({ rascunhoIds: ids });
  ...
  return (
    <main className="mx-auto flex w-full max-w-7xl flex-col gap-5 p-4 sm:p-6">
      <header className="flex flex-col gap-4 rounded-lg border bg-white p-5 shadow-xs sm:flex-row sm:items-center sm:justify-between">
        <div>
          <Link
            href="/admin/fornecedores/integracoes/laquila/conciliacao"
          ...
          <p className="mt-1 text-sm text-slate-500">
            Revise e confirme os produtos que entrarão no catálogo da loja.
          </p>
        </div>
